...
def setup(rank, world_size):
    print(f'Setup started for {rank}, {torch.cuda.is_available()}')
...
def motef_worker(rank, world_size, model, train_loader, val_loader, epochs, gamma, eta, lambda_, C_alpha):
    ...
    print(f'motef_worker rank {rank}, and device {device}')
    ...
    for epoch in range(epochs):
        print(f'worker {rank} at epoch {epoch}, with device {device}')
        train_loader.sampler.set_epoch(epoch)
        print(f'Loader finished')
        
        for batch_idx, (data, target) in enumerate(train_loader):
            data, target = data.to(device), target.to(device)
            print(f'Heeeey, shit\'s happening')
